...
const lastKnownOrderbook = new Map<
  string,
    ...
    try {
      ...
      if (message.stream && message.stream.startsWith('depth.')) {
        ...
        if (
          ...
          currentOrderbook.bestAsk !== null
        ) {
          const midPrice =
            (currentOrderbook.bestBid + currentOrderbook.bestAsk) / 2;
          const priceAsInteger = Math.round(
            midPrice * 10 ** assetInfo.decimals,
          );
...
const startPublisher = () => {
  ...
  setInterval(async () => {
    ...
    const priceUpdates = Array.from(latestPrices.entries()).map(
      ([asset, data]) => ({
        asset,
        price: data.price,
